...
public static class HttpContextExtensions
{
    ...
    public static string GetCurrentAlias(this IHttpContextAccessor source)
        => source?.HttpContext?.Request?.Headers[HttpContextKeys.ClientAliasKey].ToString() ?? string.Empty;
    ...
    public static string GetUsername(this IHttpContextAccessor source)
        => source?.HttpContext?.User?.FindFirst(HttpContextKeys.UserNameKey)?.Value ?? string.Empty;
}
