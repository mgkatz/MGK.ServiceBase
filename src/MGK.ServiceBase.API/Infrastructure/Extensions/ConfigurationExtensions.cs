    ...
    public static class ConfigurationExtensions
    {
        ...
        public static IDbServerSetup GetDatabaseSetup(this IConfiguration configuration, string clientAlias)
        {
            Ensure.Parameter.IsNotNullNorEmptyNorWhiteSpace(
                clientAlias,
                nameof(clientAlias),
                BaseResources.MessagesResources.ErrorAliasNotExist);
        ...
        public static IDictionary<string, IDbServerSetup> GetDatabaseSetupList(this IConfiguration configuration)
        {
            ...
            if (!configuration.GetValue<bool>(AppConfigurationKeys.UseMultiTenant))
            {
                ...
                configuration.GetSection(AppConfigurationKeys.DatabaseSetup).Bind(dbsInConfig);
                databasesList = dbsInConfig.ToArray();
            }
            else
            {
                var secretName = configuration.GetSection(AppConfigurationKeys.AzVaultSecretName).Value;
